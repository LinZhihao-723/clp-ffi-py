...
public:
     ...
     * both ts_lower_bound_check and ts_upper_bound_check.
     * @param ts Given timestamp.
     ...
     * range. This range is set by `cTimestampUpperBoundSafeRange`, and the
     * safe upper bound is stored in `m_ts_safe_upper_bound`.
     * @param ts Given timestamp.
     ...
     * return true as it is considered as a match. In a case where a wildcard
     * query is matched, it will be returned as a string view. If there are
     * multiple wildcard queries matched, the first one added to the list will
     * be returned.
     * @param log_message Input log message.
     * @return std::pair<bool, std::optional<std::string_view>> The first
     * element in the pair indicates whether the message matches the query,
     * and the second element in the pair returns the first matched wildcard
     * query. If no matches found, the second element is set to std::nullopt.
     */
    [[nodiscard]] auto wildcard_matches(std::string_view log_message)
            -> std::pair<bool, std::optional<std::string_view>>;

    /**
     * Validates whether the given log event matches the given query. In the
     * case of a valid matching, the timestamp of the log event must be within
     * the search range, and the message represented by the log event must
     * match at least one of the wildcard query if the wildcard query list is
     * not empty.
     * @param log_event Input log event.
     * @return std::pair<bool, std::optional<std::string_view>> The first
     * element in the pair indicates whether the log event matches the query,
     * and the second element in the pair returns the first matched wildcard
     * query. If no matches found, the second element is set to std::nullopt.
     */
    [[nodiscard]] auto matches(LogEvent const& log_event)
            -> std::pair<bool, std::optional<std::string_view>> {
